...
const PostItem = props => {
    return (
        <div key={props._id} className="posts-list-item my-5 p-4">
            ...
            <div className="list-footer">
                ...
                <div className="list-buttons d-flex justify-content-end position-relative">
                    ...
                    {(props.userId === props.creatorId || props.userRol === 'Admin') && <button className="list-edit-button btn btn-secondary mt-4">Edit post</button>}
                </div>
            </div>
        </div>
    )
...
const PostsList = props => {
    const posts = props.posts.map(post => {
        return <PostItem key={post._id} userId={props.authUserId} userRol={props.authUserRol} creatorId={post.creator._id} title={post.title} description={post.description} creator={post.creator.username} createdAt={post.createdAt} image={post.imageUrl}/>
    })
